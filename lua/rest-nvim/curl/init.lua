...
local function send_curl_start_event(data)
    vim.api.nvim_exec_autocmds('User', {pattern = 'RestStartRequest', modeline = false, data = data})
end

local function send_curl_stop_event(data)
    vim.api.nvim_exec_autocmds('User', {pattern = 'RestStopRequest', modeline = false, data = data})
end
...
M.get_or_create_buf = function()
    ...
    if existing_bufnr ~= -1 then
        ...
        vim.api.nvim_buf_set_lines(existing_bufnr, 0, vim.api.nvim_buf_line_count(existing_bufnr) - 1, false, {})
...
local function create_callback(curl_cmd, opts)
    ...
    return function(res)
        ...
        for _, value in pairs(res.headers) do
            for name, data in string.gmatch(value, '([^:]+): ?(.+)') do parsed_headers[name] = data end
        end

        local res_bufnr = M.get_or_create_buf()
        local content_type = parsed_headers[utils.key(parsed_headers, 'content-type')]
        if content_type then
            content_type = content_type:match('application/([^; ]+)') or content_type:match('text/(%l+)')
        end
        ...
        if config.get('result').show_http_info then
            ...
            vim.api.nvim_buf_set_lines(res_bufnr, line_count - separator, line_count - separator, false,
        ...
        if config.get('result').show_headers then
            ...
            vim.api.nvim_buf_set_lines(res_bufnr, line_count + 1, line_count + 1 + #res.headers, false, res.headers)
        end
        ...
        if vim.fn.bufwinnr(res_bufnr) == -1 then
            ...
            if config.get('result_split_horizontal') then cmd_split = [[sb]] end
            if config.get('result_split_in_place') then cmd_split = [[bel ]] .. cmd_split end
...
M.curl_cmd = function(opts)
    ...
    if opts.dry_run then
        if config.get('yank_dry_run') then vim.cmd('let @+=' .. string.format('%q', curl_cmd)) end
