...
local rest = {}
local Opts = {}
local defaultRequestOpts = {verbose = false, highlight = false}

local LastOpts = {}

rest.setup = function(user_configs) config.set(user_configs or {}) end
...
rest.run_file = function(filename, opts)
    ...
    defaultRequestOpts, {highlight = config.get('highlight').enabled}, opts or {})
    ...
    for _, req in pairs(requests) do rest.run_request(req, opts) end
...
local function splice_headers(headers)
    for name, value in pairs(headers) do headers[name] = utils.replace_vars(value) end
    return headers
end
...
local function splice_body(headers, payload)
    ...
    if external_payload then
        ...
        if not utils.file_exists(importfile) then error('import file ' .. importfile .. ' not found') end
    ...
    for _, line in ipairs(lines) do body = body .. utils.replace_vars(line, vars) end
    ...
    if is_json and json_body then
        if has_json then
            ...
            for key, val in pairs(json_body) do
                if type(val) == 'table' then json_body[key] = vim.fn.json_encode(val) end
            end
            return vim.fn.json_encode(json_body)
        end
    end
...
rest.run_request = function(req, opts)
    ...
    local curl_raw_args = config.get('skip_ssl_verification') and vim.list_extend(req.raw, {'-k'}) or req.raw
    opts = vim.tbl_deep_extend('force', -- use value from rightmost map
    defaultRequestOpts, {highlight = config.get('highlight').enabled}, opts or {})
    ...
    Opts = {
        ...
        script_str = req.script_str,
    }

    if not opts.verbose then LastOpts = Opts end

    if opts.highlight then backend.highlight(req.bufnr, req.start_line, req.end_line) end
...
rest.last = function()
    ...
    if config.get('highlight').enabled then backend.highlight(LastOpts.bufnr, LastOpts.start_line, LastOpts.end_line) end
