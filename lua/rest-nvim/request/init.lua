...
local function get_body(bufnr, start_line, stop_line)
    ...
    for _, line in ipairs(lines) do
        ...
        if not utils.contains_comments(line) then lines2[#lines2 + 1] = line end
    end
...
local function get_response_script(bufnr, start_line, stop_line)
    ...
    if script_start_rel == nil then return nil end
...
local function is_request_line(line)
    ...
    for _, method in ipairs(http_methods) do if line:find('^' .. method) then return true end end
    return false
end
...
local function get_headers(bufnr, start_line, end_line)
    ...
    for line_number = start_line + 1, end_line do
        ...
        if not utils.contains_comments(header_name) then headers[header_name] = header_value end
        ::continue::
    end
...
local function get_curl_args(bufnr, headers_end, end_line)
    ...
    for line_number = headers_end, end_line do
        ...
        elseif not line_content:find('^ *$') then
            if line_number ~= end_line then body_start = line_number - 1 end
            break
        end
    end
...
local function end_request(bufnr, linenumber)
    ...
    if linenumber < last_line then linenumber = linenumber + 1 end
    utils.move_cursor(bufnr, linenumber)
    ...
    if next == 0 or (oldlinenumber == last_line) then
        ...
        while vim.fn.getline(next - 1):find('^ *#') do next = next - 1 end
...
local function parse_url(stmt)
    ...
    if parsed[2] ~= nil then http_version = parsed[2] end
    ...
    return {method = http_method, http_version = http_version, url = target_url}
end

local M = {}
M.get_current_request = function() return M.buf_get_request(vim.api.nvim_win_get_buf(0), vim.fn.getcurpos()) end
...
M.buf_get_request = function(bufnr, curpos)
    ...
    if start_line == 0 then return false, 'No request found' end
...
M.print_request = function(req) print(M.stringify_request(req)) end

-- converts request into string, helpful for debug
-- full_body boolean
M.stringify_request = function(req, opts)
    opts = vim.tbl_deep_extend('force', -- use value from rightmost map
    ...
    if opts.headers then
        for name, value in pairs(req.headers) do str = str .. 'header \'' .. name .. '\'=' .. value .. '\n' end
    end
...
M.highlight = function(bufnr, start_line, end_line)
    ...
    vim.highlight.range(bufnr, select_ns, higroup, {start_line - 1, 0}, {end_line - 1, end_column},
        {regtype = 'c', inclusive = false})

    vim.defer_fn(function()
        if vim.api.nvim_buf_is_valid(bufnr) then vim.api.nvim_buf_clear_namespace(bufnr, select_ns, 0, -1) end
    end, timeout)
end
