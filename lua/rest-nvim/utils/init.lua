...
M.binary_content_types = {'octet-stream'}

M.is_binary_content_type = function(content_type) return vim.tbl_contains(M.binary_content_types, content_type) end
...
M.move_cursor = function(bufnr, line, column)
    column = column or 1
    vim.api.nvim_buf_call(bufnr, function() vim.fn.cursor(line, column) end)
end
        ...
        if M.file_exists(env_file_path) then
            ...
            if file ~= nil then
                if string.match(env_file_path, '(.-)%.json$') then
                    ...
                    for key, value in pairs(variables) do file:write(key .. '=' .. value .. '\n') end
                end
                file:close()
            end
        end
...
M.file_exists = function(file) return vim.fn.filereadable(file) == 1 end
...
M.get_variables = function()
    ...
    for k, v in pairs(file_variables) do variables[k] = v end

    for k, v in pairs(env_variables) do variables[k] = v end
...
M.read_dynamic_variables = function()
    ...
    for k, v in pairs(from_config) do dynamic_variables[k] = v end
...
M.replace_vars = function(str, vars)
    if vars == nil then vars = M.read_variables() end
    ...
    for var in string.gmatch(str, '{{[^}]+}}') do
        ...
        if M.has_key(vars, var) then
            str = type(vars[var]) == 'function' and str:gsub('{{' .. var .. '}}', vars[var]()) or
                      str:gsub('{{' .. var .. '}}', vars[var])
        else
...
M.has_key = function(tbl, key)
    for tbl_key, _ in pairs(tbl) do if string.find(key, tbl_key) then return true end end
    return false
end
...
M.has_value = function(tbl, str)
    for _, element in ipairs(tbl) do if string.find(str, element) then return true end end
    return false
end
...
M.key = function(tbl, key)
    for tbl_key, _ in pairs(tbl) do if string.lower(tbl_key) == string.lower(key) then return tbl_key end end
    return key
end
...
M.char_to_hex = function(char) return string.format('%%%02X', string.byte(char)) end

-- encode_url encodes the given URL
-- @param url The URL to encode
M.encode_url = function(url)
    if url == nil then error('You must need to provide an URL to encode') end
...
-- @return number
M.contains_comments = function(str) return str:find('^#') or str:find('^%s+#') end
...
M.http_status = function(code)
    ...
    if status_meaning[code] ~= nil then return tostring(code) .. ' ' .. status_meaning[code] end
